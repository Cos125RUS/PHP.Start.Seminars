  ...
  crossorigin="anonymous"></script>
    ...
    <body class="d-flex flex-column h-100">
        ...
        <footer class="footer mt-auto py-3 bg-light">
          <div class="container d-flex justify-content-between bd-highlight mb-2">
            <span class="text-muted">Место для контента прикрепленного футера здесь.</span>
              <p class="text-muted" id="time">{{ time | date("H:i:s") }}</p>
              <script>
                  addEventListener("DOMContentLoaded", () => {
                      setInterval(() => {
                          $.ajax({
                              ...
                              const print = `${time.getHours()}:${time.getMinutes()}:${time.getSeconds()}`;
                              timeBox.innerHTML = print;
                          });
                      }, 1000);
                  });
              </script>
